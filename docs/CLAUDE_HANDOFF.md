...
### 행정규칙 본문에만 있는 검색어가 항상 0건으로 빠지던 문제

원인: 행정규칙(고시·훈령·예규 등) 상세조회 응답은 법령·자치법규와 JSON 구조가 다르다. 법령은 `조문 > 조문단위`(조·항·호·목이 각각 객체)인데, 행정규칙은 `조문내용`이 조 전체를 하나의 문자열로 합친 리스트다. `detail._article_units()`가 이 평평한 문자열 리스트 형태를 인식하지 못해 본문 조·항·호·목 후보를 하나도 만들지 못했고, 그 결과 `SearchService._verify_body_results()`가 "정확 일치 문맥 없음"으로 판단해 검색어가 제목에 없는 행정규칙을 전부 결과에서 제외했다. 검색어가 제목에 있는 행정규칙(제목 일치)은 본문 검증을 거치지 않으므로 영향이 없었다.

실사례로 확인: `방화구획`, `주차장법` 검색에서 law.go.kr 자체 AI 검색(`/LSW/ais/main.do`)에는 뜨는 행정규칙 본문 일치(예: 교육부 고시 「교육시설의 안전·유지관리기준」 제13조·제2조의 "방화구획")가 이 앱에서는 행정규칙 0건으로 나왔다.

수정: `detail._article_units()`가 `조문내용`이 문자열 리스트인 경우 각 문자열을 `{"조문내용": 문자열}` 형태의 단위로 취급하도록 분기 추가. 이미 있던 "조문내용에서 제목 정규식으로 조 번호·제목을 추출하는" 로직과 본문 직접 일치 로직을 그대로 재사용하므로 별도 파싱 코드는 필요 없었다.

회귀 근거: tests/test_detail.py의 test_admin_rule_flat_article_content_list_returns_article_heading. 실제 법제처 API에서 받아온 「교육시설의 안전·유지관리기준」 상세 응답으로도 수동 검증(제2조·제12조·제13조·제14조에서 "방화구획" 문맥 정상 추출 확인).
...
5. 좁은 폭(375px) 수동 검증: 사이드바가 접히고 카드 2열이 1열 전체 폭으로 재배치되며, 카드 본문·버튼이 모두 읽을 수 있는 상태로 유지됨을 확인. 모바일 UX는 여전히 낮은 우선순위이며 별도 최적화는 하지 않음.
